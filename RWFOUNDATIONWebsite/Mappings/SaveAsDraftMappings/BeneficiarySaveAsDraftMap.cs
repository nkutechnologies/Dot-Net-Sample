...
{
    public class BeneficiarySaveAsDraftMap : IEntityTypeConfiguration<BeneficiaryFormSaveAsDraft>
    {
        public void Configure(EntityTypeBuilder<BeneficiaryFormSaveAsDraft> builder)
        {
            builder.HasKey(g => g.GroceryKitId);
            builder.Property(g => g.GroceryKitId);
            builder.Property(g => g.FormNo).HasColumnName("FormNo");
            ...
            builder.HasOne(c => c.Occupation).WithMany(c => c.BeneficiaryFormSaveAsDrafts).HasForeignKey(x => x.OccupationId).OnDelete(DeleteBehavior.Restrict);
            builder.HasOne(c => c.MedicalProb).WithMany(c => c.BeneficiaryFormSaveAsDrafts).HasForeignKey(x => x.MedicalProbId).OnDelete(DeleteBehavior.Restrict);
            builder.HasOne(c => c.CurrentStatus).WithMany(c => c.BeneficiaryFormSaveAsDrafts).HasForeignKey(x => x.CurrentStatusId).OnDelete(DeleteBehavior.Restrict);
            builder.HasOne(c => c.City).WithMany(c => c.BeneficiaryFormSaveAsDrafts).HasForeignKey(x => x.CityId).OnDelete(DeleteBehavior.Restrict);
            builder.HasOne(c => c.Province).WithMany(c => c.BeneficiaryFormSaveAsDrafts).HasForeignKey(x => x.ProvinceId).OnDelete(DeleteBehavior.Restrict);
        }
    }
}
